...
{
    // Aryacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Aryacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
//komodo
    ...
    // Aryacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1550751039;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1646836;
        ...
        assert(block.hashMerkleRoot == uint256("0x0e1eea9e9094da5741931085408bfced4d9387eb5d3737a2b1515f18a19cc381"));
...
unsigned char pchMessageStart[4] = { 0x88, 0xe1, 0xea, 0xe7 }; // Aryacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AryacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AryacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AryacoinMiner(CWallet *pwallet)
{
    printf("AryacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Aryacoin-miner");
    ...
    try { loop {
        ...
        printf("Running AryacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AryacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AryacoinMiner, pwallet));
}
